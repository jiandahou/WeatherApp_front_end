...
export function Buttonforoneday({weatherForThatDay,isActive=false,onClick=()=>{return}}:{
    ...
    return(
        <div className={clsx("shrink-0 grow-0",{"basis-36":isActive==false,"basis-72 ":isActive==true})}>
            <button className={clsx("h-28 border border-black rounded-lg px-2 button w-full",{" bg-white-transparent":isActive==false,"bg-white":isActive==true})} onClick={(e)=>onClick()}>
                <div>{monthIntepretor[weatherForThatDay.time.getMonth()]} {weatherForThatDay.time.getDate().toString()}</div>
                <div>{weekdayIntepretor[weatherForThatDay.time.getDay()]}</div>
                <div className="flex">
                    <img alt="weathername.svg" src={weathername+".svg" } width={40} height={40}></img>
                    <div className="ml-8 flex-1">
                        <div className="flex relative">
                            <div>{weatherForThatDay.highestTemperature.toFixed(1)}°</div>
                            {isActive&&<div className="absolute right-0">{weathername}</div>}
                        </div>
                        <div className="flex relative">
                            <div>{weatherForThatDay.lowestTemperature.toFixed(1)}°</div>
                            {isActive&&<div className="absolute right-0">{weatherForThatDay.recipitationProbabilityMax}%</div>}
                        </div>
                    </div>
                </div>
            </button>
...
export function ButtonPanleForTenDay({onClick}:{onClick:Function}){
    ...
    return(
        <div ref={divref} className="relative my-5">
            ...
            {!(indexOnpage==0)&&
            <button title="turnleft" className="absolute left-0 top-1/2 rotate-180 z-50" onClick={(e)=>{turnleft()}}>
            ...
            <button title="turnright" className="absolute right-0 top-1/2" onClick={(e)=>{turnright()}}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M7.57107 11.8403C6.90803 12.2987 6 11.8271 6 11.0244V4.97557C6 4.17283 6.90803 3.70129 7.57106 4.1597L11.3555 6.77618C12.2133 7.3693 12.2134 8.63066 11.3555 9.22378L7.57107 11.8403Z" fill="#1A1A1A"></path></svg>
            </button>}
            </div>
        </div>
